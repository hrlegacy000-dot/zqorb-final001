(() => {
  ...
  const FALLBACK_PROJECTS = [
    {
      ...
      "category": "GAME / ACTIVE DEVELOPMENT",
      "status": "IN DEVELOPMENT",
      "description": "A psychological action-horror game set inside Astraeon High, where a supernatural curse turns negativity into a threat.",
      "tags": ["GAME", "HORROR", "UE5", "STORY"],
      "folder": "projects/detention/",
      "image": "projects/detention/images/hero.jpg"
    },
    {
      "id": "build-mode",
      "number": "02",
      ...
      "status": "BUILDING",
      "description": "A practical learning system designed to move students from understanding technology to actually building with it.",
      "tags": ["AI", "CODE", "SAAS", "AUTOMATION"],
      "folder": "projects/build-mode/",
      "image": "projects/build-mode/images/hero.jpg"
    },
    {
      "id": "neuronix",
      "number": "03",
      "title": "NEURONIX",
      "subtitle": "PERSONAL AI / TECHNOLOGY",
      "category": "TECHNOLOGY / R&D",
      "status": "EXPLORATION",
      "description": "A personal AI concept focused on a compact, synchronized intelligence layer across phone and computer.",
      "tags": ["AI", "SYSTEMS", "R&D", "PRODUCT"],
      "folder": "projects/neuronix/",
      "image": "projects/neuronix/images/hero.jpg"
    }
  ];
  ...
  // Hand-built visual identity per project.
  // Build Mode and Neuronix keep their original SVG visuals.
  // DETENTION uses the actual DETENTION logo.
  const ART = {
    detention: `
      <svg viewBox="0 0 400 540" preserveAspectRatio="xMidYMid slice" role="img" aria-label="">
        ...
        <circle cx="182" cy="199" r="3.4" fill="#A6423C">
          <animate
    ...
    "build-mode": `
      <svg viewBox="0 0 400 540" preserveAspectRatio="xMidYMid slice" role="img" aria-label="">
        ...
        <g fill="#6a5fa0">
          ...
          <circle cx="280" cy="195" r="3"/>
          <circle cx="280" cy="285" r="3"/>
          <circle cx="200" cy="330" r="3"/>
          <circle cx="120" cy="285" r="3"/>
          <circle cx="120" cy="195" r="3"/>
          <circle cx="200" cy="240" r="3.4"/>
        </g>
    ...
    neuronix: `
      <svg viewBox="0 0 400 540" preserveAspectRatio="xMidYMid slice" role="img" aria-label="">
        ...
        <circle cx="200" cy="250" r="5" fill="#C7BBFF">
          <animate
            attributeName="r"
            values="4;6;4"
            dur="2.6s"
  ...
  function renderProjects() {
    // Cards are real links to each project's own page.
    grid.innerHTML = projects.map((p, i) => `
      <a
        class="project-card reveal ${p.id === "build-mode" ? "lav pc-2" : (i === 0 ? "pc-1" : "pc-3")}"
        href="/projects/${p.id}/"
        style="--delay:${i * 90}ms"
      >
        ...
        <div class="project-visual">
          ${
            p.id === "detention"
              ? `<img
                  class="detention-logo"
                  src="/projects/detention/images/detention-logo.png"
                  alt="DETENTION logo"
                >`
              : ART[p.id] || ""
          }
    ...
    grid
      ...
      .forEach(card => window.ZQORB.observe(card));
  }
  ...
  function renderTabs() {
    tabs.innerHTML = projects
      .map(
        (p, i) =>
          `<button class="ws-tab${i === 0 ? " active" : ""}" data-id="${p.id}">
            ${p.number} ${p.title}
          </button>`
      )
      .join("");
  ...
  function showProject(id) {
    ...
    media.innerHTML = `
      <div class="media-placeholder">
        ...
        <small>Add hero.jpg to ${p.folder}images/</small>
      </div>
    `;
    ...
    docList.innerHTML = `
      <span class="empty">
        Preview only — permanent files belong in ${p.folder}documents/
      </span>
    `;

    imageList.innerHTML = `
      <span class="empty">
        Preview only — permanent images belong in ${p.folder}images/
      </span>
    `;
  }
